...
export default function BreveCatecismo() {
  const [searchTerm, setSearchTerm] = useState('')
  const [expandedItems, setExpandedItems] = useState({})
  const [filteredCatecismo, setFilteredCatecismo] = useState(breveCatecismo)
  const [isSearching, setIsSearching] = useState(false)
  
  useEffect(() => {
    if (searchTerm.trim() === '') {
      setFilteredCatecismo(breveCatecismo)
      return
    }
    
    const searchTermLower = searchTerm.toLowerCase()
    const filtered = breveCatecismo.filter(item => 
  ...
  return (
    <main className="min-h-screen">
      ...
      <div className="sm:ml-64 max-w-4xl mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-8 text-center">Breve Catecismo de Westminster</h1>
